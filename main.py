...
def main():
    ...
    screen = p.display.set_mode(
        (BOARD_WIDTH + MOVE_LOG_PANEL_WIDTH + 10, BOARD_HEIGHT))
    clock = p.time.Clock()
    screen.fill(p.Color(LIGHT_SQUARE_COLOR))
        ...
        for e in p.event.get():
            ...
            # Mouse Handler
                ...
                if not gameOver:  # allow mouse handling only if its not game over
                    ...
                    col = (location[0] - BAR_WIDTH)//SQ_SIZE
...
def drawGameState(screen, gs, validMoves, squareSelected, moveLogFont):
    ...
    draw_bar(screen, white_ratio)
import random
CHECKMATE = 25
def updateBarOnMove(gs):
    """
    Update the white_ratio based on the current game state score.
    """
    #print('hi')
    score = scoreBoard(gs)  # Get the current board score
    global white_ratio
    # Normalize the score to a ratio between 0 and 1
    # Assuming CHECKMATE is 1000 and -CHECKMATE is -1000
    maxScore = CHECKMATE
    minScore = -CHECKMATE
    white_ratio = (score - minScore) / (maxScore - minScore)  # Normalize to [0, 1]
    print("User's chances of winning:", white_ratio)
    # Update the bar with the calculated ratio
    #draw_bar(screen, white_ratio)  # Update only when this function is called

def draw_bar(screen, white_ratio):
    """Draw the vertical bar on the left side."""
    # Calculate bar heights
    WHITE = (245, 245, 245)
    BLACK = (55, 55, 55)
    #WHITE = (255, 255, 255)
    #BLACK = (0, 0, 0)
    white_height = int(MOVE_LOG_PANEL_HEIGHT * white_ratio)
    black_height = MOVE_LOG_PANEL_HEIGHT - white_height

    # Draw the white and black portions of the bar
    p.draw.rect(screen, BLACK, p.Rect(0, 0, BAR_WIDTH, black_height))  # Black section on top
    p.draw.rect(screen, WHITE, p.Rect(0, black_height, BAR_WIDTH, white_height))  # White section on bottom

    # Add a border to the bar
    p.draw.rect(screen, WHITE, p.Rect(0, 0, BAR_WIDTH, MOVE_LOG_PANEL_HEIGHT), 3)
    ...
    for row in range(DIMENSION):
        for col in range(DIMENSION):
            ...
            p.draw.rect(screen, color, p.Rect(
                BAR_WIDTH + col * SQ_SIZE, row * SQ_SIZE, SQ_SIZE, SQ_SIZE))
...
def highlightSquares(screen, gs, validMoves, squareSelected):
    if squareSelected != ():  # make sure there is a square to select
        ...
        if gs.board[row][col][0] == ('w' if gs.whiteToMove else 'b'):
            ...
            screen.blit(s, (BAR_WIDTH + col*SQ_SIZE, row*SQ_SIZE))
            # highlighting valid square
            s.fill(p.Color(POSSIBLE_MOVE_COLOR))
            for move in validMoves:
                if move.startRow == row and move.startCol == col:
                    screen.blit(s, (BAR_WIDTH + move.endCol*SQ_SIZE, move.endRow*SQ_SIZE))
...
def drawMoveLog(screen, gs, font):
    # rectangle
    moveLogRect = p.Rect(
        BOARD_WIDTH + BAR_WIDTH, 0, MOVE_LOG_PANEL_WIDTH, MOVE_LOG_PANEL_HEIGHT)
...
def animateMove(move, screen, board, clock):
    ...
    for frame in range(frameCount + 1):
        ...
        endSquare = p.Rect(BAR_WIDTH + move.endCol*SQ_SIZE, move.endRow *
        ...
        if move.pieceCaptured != '--':
            if move.isEnpassantMove:
                ...
                endSquare = p.Rect(BAR_WIDTH + move.endCol*SQ_SIZE, enPassantRow *
                                   SQ_SIZE, SQ_SIZE, SQ_SIZE)  # pygame rectangle
